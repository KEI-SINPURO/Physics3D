const physicsData = {
    ...
    "c6": {
        ...
        formulas: [
            ...
            { name: "向心力", math: "$$F=mr\\omega^2=m\\frac{v^2}{r}$$", usage: "円運動を維持する力（張力や摩擦力など）。", reason: "運動方程式(ma=F)に向心加速度を代入したものです。", animType: "circular_f", simText: "【赤矢印:向心力】ひもが引く力などが向心力となって円運動を維持します。" },
    ...
    "c10": {
        ...
        formulas: [
            { name: "ボイル・シャルルの法則", math: "$$\\frac{PV}{T}=\\text{一定}$$", usage: "気体の圧力、体積、温度のどれかが変化した時。", reason: "閉じ込められた一定量の気体で成り立ちます。", animType: "boyle", simText: "体積を圧縮すると圧力が上がり、温めると膨張します。" },
    ...
    "c12": {
        ...
        formulas: [
            ...
            { name: "弦・気柱の固有振動", math: "$$f_m=m\\frac{v}{2l} \\text{ (開)} \\quad f_m=m\\frac{v}{4l} \\text{ (閉)}$$", usage: "楽器が発する特定の音の高さ。", reason: "両端の固定端・自由端の条件を満たす定常波しか存在できません。", animType: "standing", simText: "両端で反射した波が重なり、定常波（動かない波）を作ります。" },
            { name: "ドップラー効果", math: "$$f'=\\frac{V-v_o}{V-v_s}f$$", usage: "動く音源や観測者による音の高さの変化。", reason: "音源が動くと波長が縮み、観測者が動くと波の受け取り回数が変わります。", animType: "doppler", simText: "音源の進行方向は波面が詰まって波長が短く（高音に）なります。" }
        ]
    },
    "c13": {
    ...
    "c15": {
        ...
        formulas: [
            ...
            { name: "合成容量", math: "$$C_{\\text{並}}=C_1+C_2 \\quad \\frac{1}{C_{\\text{直}}}=\\frac{1}{C_1}+\\frac{1}{C_2}$$", usage: "複数繋いだときの全体の容量。", reason: "並列は極板面積が広がる効果、直列は間隔が広がる効果です。", animType: "cap_circuit", simText: "繋ぎ方によって、全体の電荷の溜まりやすさ（容量）が変化します。" }
        ]
    },
    "c16": {
        chap: "電磁気", title: "16 電流と直流回路",
        formulas: [
            { name: "電流の定義と微視的モデル", math: "$$I=\\frac{q}{t} \\quad I=envS$$", usage: "1秒間に通過する電気量と、電子の運動からの導出。", reason: "電流の正体は電子の流れです。", animType: "current", simText: "導線の中を自由電子（黄色）が移動することで電流になります。" },
            ...
            { name: "キルヒホッフの法則", math: "$$\\Sigma I_{\\text{in}}=\\Sigma I_{\\text{out}} \\quad \\Sigma V=\\Sigma RI$$", usage: "複雑な回路を解く時。", reason: "電荷の保存とエネルギー(電位)の保存の法則です。", animType: "kirchhoff", simText: "分岐点で電流は分かれ、回路を1周すると電位の上がり下がりは0になります。" }
        ]
    },
    "c17": {
    ...
    "c20": {
        ...
        formulas: [
            ...
            { name: "光電方程式", math: "$$K_{\\max}=h\\nu-W$$", usage: "光を当てて飛び出した電子の運動エネルギー。", reason: "もらったエネルギー(hν)から、金属から脱出する仕事関数(W)を引いた残りです。", animType: "photoelectric", simText: "光の粒（フォトン）が衝突し、金属から電子を弾き飛ばします。" },
    ...
    "c21": {
        ...
        formulas: [
            { name: "量子条件", math: "$$mvr=n\\frac{h}{2\\pi} \\quad (2\\pi r=n\\lambda)$$", usage: "電子が光を出さずに安定して回れる特定の軌道。", reason: "電子の波（物質波）が軌道上で綺麗に繋がる（定常波になる）ためです。", animType: "bohr_orbit", simText: "電子の波がぴったり1周で繋がる（定常波をつくる）特定の軌道だけが存在できます。" },
    ...
    "c22": {
        ...
        formulas: [
            ...
            { name: "半減期", math: "$$N=N_0\\left(\\frac{1}{2}\\right)^{\\frac{t}{T}}$$", usage: "時間tの後に残っている放射性同位体の数。", reason: "半減期Tごとに数が半分になるため、指数関数になります。", animType: "half_life", simText: "時間が経つごとに、元の原子核の数が半減していきます（グラフ）。" },
